...
def visualize_multi(opt, real_A, model, eidx, uidx):
    ...
    with torch.no_grad():
        multi_prior_z_B = real_A.data.new(opt.num_multi,opt.nlatent, 1, 1).normal_(0, 1).repeat(size[0],1,1,1)
...
def train_model():
    ...
    if opt.seed is not None:
        print ("using random seed: %d" % opt.seed)
    ...
    if opt.numpy_data:
        ...
        #print(type(AlignedIterator(devA, devB, batch_size = 100)))
        dev_cycle = itertools.cycle(AlignedIterator(devA, devB, batch_size=10))
    else:
