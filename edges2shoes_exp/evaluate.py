...
def eval_mse_A(dataset, model, use_gpu=True):
    ...
    for batch in dataset:
        ...
        F_mse_loss = F.mse_loss(pred_A, real_A).item()
        mse_A.append(F_mse_loss)
    
    print('len mseA: ', len(mse_A))
...
def variational_ubo(model, real_A, real_B, steps, visualize=False, vis_name=None, vis_path=None,
    ...
    if compute_l1:
        if model.opt.stoch_enc:
            ...
            with torch.no_grad():
                x = mu.view(mu.size(0), mu.size(1), 1, 1).data
    ...
    for i in range(steps):
        ...
        ubo_val_new = ubo.mean(0).item()
        kld_val = kld.mean(0).item()
        bpp = ubo.mean(0).item() / (64*64*3* math.log(2.))
        if compute_l1:
            l1_loss = F.l1_loss(real_B, rec_B).mean(0).item()
        if verbose:
